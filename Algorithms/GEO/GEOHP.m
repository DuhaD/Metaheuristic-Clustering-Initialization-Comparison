...
%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fval,ConvergenceCurve,x] = GEO (nvars,fname,maxIterations,pop)
...
CostFunction = @(y) GetFunction(y ,fname);        % Cost Function

%x = lb + rand (PopulationSize,nvars) .* (ub-lb);
x=initialization_with_kmeans_plus(PopulationSize,nvars,1,0,fname);


FitnessScores =[];
[rows columns] = size(x);
for i = 1:rows
  Cost=CostFunction(x(i, :));  
  FitnessScores = [FitnessScores;Cost ];
end
...
for CurrentIteration = 1 : MaxIterations
	...
	% calculate fitness
	%FitnessScores = @(x) GetFunction(x ,fname);
