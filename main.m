...
for Algo = 1:Algo_num %iterate over the four algorithms
    ...
    filename=strcat('Results/Experiments',trimmed,'-');
    ...
    for d= 1:numdatasets
        ...
        parfor i =1:NumberOfRuns
            ...
            if (Algo == 1)
                 tic();
                [gf ,generations_best,best_position] = ga(dimentions{d}, Dataset{d},MaxIteration, AgentsNum );
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 2)
                tic();
                [gf ,generations_best,best_position] = pso(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 3)
                tic();
                [gf ,generations_best,best_position] = de(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});

            elseif(Algo == 4)
                tic();
                [gf ,generations_best,best_position]=SSA(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 5)
                tic();
                [gf ,generations_best,best_position]=HHO(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            
            elseif(Algo == 6)
                tic();
                [gf ,generations_best,best_position]=abc(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});

            elseif(Algo == 7)
                tic(); 
                [gf ,generations_best,best_position]=HBO(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 8)
                tic();
                [gf ,generations_best,best_position]=GEO(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
