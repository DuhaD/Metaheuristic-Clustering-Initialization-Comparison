...
for Algo = 1:Algo_num %iterate over the four algorithms
    ...
    filename=strcat('Resultshp/Experiments',trimmed,'-');
    ...
    for d= 1:numdatasets
        ...
        parfor i =1:NumberOfRuns
            ...
            if (Algo == 1)
                 tic();
                [gf ,generations_best,best_position] = gahp(dimentions{d}, Dataset{d},MaxIteration, AgentsNum );
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 2)
                tic();
                [gf ,generations_best,best_position] = psohp(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 3)
                tic();
                [gf ,generations_best,best_position] = dehp(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});

            elseif(Algo == 4)
                tic();
                [gf ,generations_best,best_position] = SSAhp(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 5)
                tic();
                [gf ,generations_best,best_position] = HHOhp(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});

            elseif(Algo == 6)
                tic();
                [gf ,generations_best,best_position]=abchp(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});

            elseif(Algo == 7)
                tic();
                [gf ,generations_best,best_position]=HBOHP(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
            elseif(Algo == 8)
                tic();
                [gf ,generations_best,best_position]=GEOHP(dimentions{d}, Dataset{d}, MaxIteration,AgentsNum);
                elapsed_time(i) = toc;
                [silhr, inter_dist]=GetSilhouetteFunction(best_position ,  Dataset{d});
